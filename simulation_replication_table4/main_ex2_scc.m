function main_ex2_scc(ncores,ind)
...
save(['./Results/crossfit_ex2_dat_',DGP,'n',num2str(n),'_ind',num2str(ind)])
end
...
lb = [-3,-3,-3,-3];
ub = [3,3,0,0];
thetahat = fmincon(fun,[0,0,0,0],[],[],[],[],lb,ub);
end
...
lb = [-3,-3];
ub = [0,0];
[betahat,fval] = fmincon(fun,[-1,-1],[],[],[],[],lb,ub);
